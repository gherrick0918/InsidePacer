...
import androidx.health.connect.client.HealthConnectClient.SdkAvailabilityStatus
...
class HealthConnectRepoImplTest {
    ...
    fun `sdk available is installed`() {
        val availability = mapSdkStatusToAvailability(SdkAvailabilityStatus.SDK_AVAILABLE)
    ...
    fun `sdk requires install is supported not installed`() {
        val installStatuses = listOf(
            SdkAvailabilityStatus.SDK_UNAVAILABLE_PROVIDER_INSTALLATION_REQUIRED,
    ...
    fun `unsupported statuses map to not supported`() {
        val unsupportedStatuses = listOf(
            ...
            SdkAvailabilityStatus.SDK_UNAVAILABLE_APP_NOT_VERIFIED,
            SdkAvailabilityStatus.SDK_UNAVAILABLE_PROVIDER_POLICY_RESTRICTION,
        )
